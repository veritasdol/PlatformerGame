...
class Editor:
	def __init__(self, land_tiles): 
		...
		# Player
		CanvasObject(pos=(200,WINDOW_HEIGHT/2), 
			frames=self.animations[0]['frames'], 
			tile_id= 0,origin=self.origin, 
			group=self.canvas_objects)

		# sky
		self.sky_handle = CanvasObject(pos=(WINDOW_WIDTH/2, WINDOW_HEIGHT/2), 
			frames=[self.sky_handle_surf], tile_id=1, origin=self.origin, group=self.canvas_objects)
			...
			if cell in self.canvas_data:
				...
				for name, side in NEIGHBOR_DIRECTIONS.items():
					...
					if neighbour_cell in self.canvas_data:
						# water top neighbors
						if self.canvas_data[neighbour_cell].has_water and self.canvas_data[cell].has_water and name == 'A':
	...
	def preview(self):
		...
		if not self.menu.rect.collidepoint(mouse_pos()):
			if selected_object:
				...
				width = 3
				size = 15
				pygame.draw.lines(self.display_surface, color, False, ((rect.left, rect.top + size),
				 rect.topleft, (rect.left + size, rect.top)), width)
				pygame.draw.lines(self.display_surface, color, False, ((rect.right - size, rect.top),
				 rect.topright, (rect.right, rect.top + size)), width)
				pygame.draw.lines(self.display_surface, color, False, ((rect.right, rect.bottom - size),
				 rect.bottomright, (rect.right - size, rect.bottom)), width)
				pygame.draw.lines(self.display_surface, color, False, ((rect.left + size, rect.bottom),
				 rect.bottomleft, (rect.left, rect.bottom - size)), width)
	...
	def display_sky(self, dt):
		...
		if y > 0:
			sea_rect = pygame.Rect(0, y, WINDOW_WIDTH, WINDOW_WIDTH)
	...
	def create_clouds(self, event):
		if event.type == self.cloud_timer:
			...
			self.current_clouds.append({'surf':surf, 'pos': pos, 'speed': randint(10, 30)})
