...
class Menu:
        ...
        for key, value in EDITOR_DATA.items():
            if value['menu']:
                if not value['menu'] in self.menu_surfs:
                    self.menu_surfs[value['menu']] = [(key, load(value['menu_surf'][1:]))]
                else:
                    self.menu_surfs[value['menu']].append((key, load(value['menu_surf'][1:])))
    ...
    def click(self, mouse_pos, mouse_button):
        for sprite in self.buttons:
            if sprite.rect.collidepoint(mouse_pos):
                if mouse_button[1]: # middle
                    if sprite.items['alt']:
                        sprite.main_active = not sprite.main_active 
                if mouse_button[2]: # right
                    sprite.switch()
    ...
    def highlight_indicator(self, index):
        ...
        if 'palm' in EDITOR_DATA[index]['menu']:
